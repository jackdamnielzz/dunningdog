...
| Supabase Auth integration | 100% | Google OAuth flow fully working in production. Two issues fixed 2026-02-27: (1) Post-login redirect race fixed by replacing `router.replace()` with `window.location.assign()`. (2) Supabase Dashboard `site_url` changed from `http://localhost:3000` to `https://dunningdog.vercel.app` and production callback URL added to `uri_allow_list` via Management API. Auth guard at `/app` layout level. Cookie-based session via `sb-auth-token`. |
