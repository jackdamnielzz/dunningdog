...
| Supabase Auth integration | 95% | Google OAuth flow code-complete and deployed. Post-login redirect bug fixed 2026-02-27 (replaced `router.replace()` with `window.location.assign()`). Auth guard at `/app` layout level. Cookie-based session via `sb-auth-token`. **Blocking issue 2026-02-27:** Supabase Dashboard Site URL misconfigured as `http://localhost:3000` — causes `bad_oauth_state` error and localhost redirect in production. Fix: change Site URL to `https://dunningdog.vercel.app` and add redirect URLs in Supabase Dashboard. No code changes needed. |
