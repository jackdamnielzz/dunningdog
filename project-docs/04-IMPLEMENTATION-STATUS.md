...
| Supabase Auth integration | 100% | Google OAuth flow code-complete and deployed. Post-login redirect bug fixed 2026-02-27: replaced `router.replace()` with `window.location.assign()` to prevent cookie/SSR race. Auth guard enforced at `/app` layout level. Cookie-based session via `sb-auth-token`. Session endpoint logs `auth.getUser()` errors. Vercel `SUPABASE_ANON_KEY` env var fixed and redeployed 2026-02-27. |
